...
import com.github.lucbui.fracktail3.discord.event.DiscordSupportedEventFactory;
...
public class DefaultDiscordOnEventHandler extends IdStore<DiscordEventHook<?>> implements DiscordOnEventHandler {
    private DiscordSupportedEventFactory factory = new DiscordSupportedEventFactory();
    /**
    ...
    public Mono<Void> execute(Bot bot, DiscordConfiguration configuration, Event event) {
        DiscordEventContext ctx = new DiscordEventContext(configuration, factory.forEvent(event));
